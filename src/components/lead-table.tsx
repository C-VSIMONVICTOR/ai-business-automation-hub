...
type Lead = { id: string; name: string; email: string; company?: string; message: string; status: string; ai_score: number; ai_temperature: string; ai_category: string; ai_recommended_action: string; created_at: string };
...
export default function LeadTable() {
  ...
  const average = leads.length ? Math.round(leads.reduce((sum, lead) => sum + lead.ai_score, 0) / leads.length) : 0;

  return <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-xl">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-xl font-semibold">Recent leads</h2><p className="mt-1 text-sm text-slate-400">Live records from the lead API.</p></div><button onClick={() => void load()} className="rounded-lg border border-slate-700 px-3 py-2 text-sm">Refresh</button></div>
